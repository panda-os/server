...
/**
 * Base class that represents a row from the 'entry_server_node' table.
 *
 * 
 *
 * @package Core
 * @subpackage model.om
 */
abstract class BaseEntryServerNode extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the server_node_id field.
	 * @var        int
	 */
	protected $server_node_id;
	...
	/**
	 * The value for the server_type field.
	 * @var        int
	 */
	...
	/**
	 * Get the [server_node_id] column value.
	 * 
	 * @return     int
	 */
	...
	/**
	 * Get the [server_type] column value.
	 * 
	 * @return     int
	 */
	...
	{
		if(!isset($this->oldColumnsValues[EntryServerNodePeer::ID]))
			$this->oldColumnsValues[EntryServerNodePeer::ID] = $this->id;
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = EntryServerNodePeer::ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[EntryServerNodePeer::ENTRY_ID]))
			$this->oldColumnsValues[EntryServerNodePeer::ENTRY_ID] = $this->entry_id;
		...
		if ($this->entry_id !== $v) {
			$this->entry_id = $v;
			$this->modifiedColumns[] = EntryServerNodePeer::ENTRY_ID;
		}
	...
	/**
	 * Set the value of [server_node_id] column.
	 * 
	 * @param      int $v new value
	 * @return     EntryServerNode The current object (for fluent API support)
	 */
	public function setServerNodeId($v)
	{
		if(!isset($this->oldColumnsValues[EntryServerNodePeer::SERVER_NODE_ID]))
			$this->oldColumnsValues[EntryServerNodePeer::SERVER_NODE_ID] = $this->server_node_id;

		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->server_node_id !== $v) {
			$this->server_node_id = $v;
			$this->modifiedColumns[] = EntryServerNodePeer::SERVER_NODE_ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[EntryServerNodePeer::PARTNER_ID]))
			$this->oldColumnsValues[EntryServerNodePeer::PARTNER_ID] = $this->partner_id;
		...
		if ($this->partner_id !== $v) {
			$this->partner_id = $v;
			$this->modifiedColumns[] = EntryServerNodePeer::PARTNER_ID;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = EntryServerNodePeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = EntryServerNodePeer::UPDATED_AT;
			}
		} // if either are not null
	...
	{
		if(!isset($this->oldColumnsValues[EntryServerNodePeer::STATUS]))
			$this->oldColumnsValues[EntryServerNodePeer::STATUS] = $this->status;
		...
		if ($this->status !== $v) {
			$this->status = $v;
			$this->modifiedColumns[] = EntryServerNodePeer::STATUS;
		}
	...
	{
		if(!isset($this->oldColumnsValues[EntryServerNodePeer::SERVER_TYPE]))
			$this->oldColumnsValues[EntryServerNodePeer::SERVER_TYPE] = $this->server_type;

		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->server_type !== $v) {
			$this->server_type = $v;
			$this->modifiedColumns[] = EntryServerNodePeer::SERVER_TYPE;
		}
	...
	public function setCustomData($v)
	{
		...
		if ($this->custom_data !== $v) {
			$this->custom_data = $v;
			$this->modifiedColumns[] = EntryServerNodePeer::CUSTOM_DATA;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		...
		try {
			...
			$this->server_node_id = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		EntryServerNodePeer::setUseCriteriaFilter(false);
		$criteria = $this->buildPkeyCriteria();
		EntryServerNodePeer::addSelectColumns($criteria);
		$stmt = BasePeer::doSelect($criteria, $con);
		EntryServerNodePeer::setUseCriteriaFilter(true);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				EntryServerNodePeer::doDelete($this, $con);
				$this->postDelete($con);
		...
		try {
			...
			for ($retries = 1; $retries < KalturaPDO::SAVE_MAX_RETRIES; $retries++)
			{
               $affectedRows = $this->doSave($con);
                if ($affectedRows || !$this->isColumnModified(EntryServerNodePeer::CUSTOM_DATA)) //ask if custom_data wasn't modified to avoid retry with atomic column 
                	break;
				...
				$validUpdate = true;
				$atomicCustomDataFields = EntryServerNodePeer::getAtomicCustomDataFields();
			...
			$this->postSave($con);
			EntryServerNodePeer::addInstanceToPool($this);
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = EntryServerNodePeer::doInsert($this, $con);
					...
					$affectedObjects = EntryServerNodePeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = EntryServerNodePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = EntryServerNodePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = EntryServerNodePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getEntryId(),
			$keys[2] => $this->getServerNodeId(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setServerNodeId($arr[$keys[2]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(EntryServerNodePeer::ID)) $criteria->add(EntryServerNodePeer::ID, $this->id);
		if ($this->isColumnModified(EntryServerNodePeer::ENTRY_ID)) $criteria->add(EntryServerNodePeer::ENTRY_ID, $this->entry_id);
		if ($this->isColumnModified(EntryServerNodePeer::SERVER_NODE_ID)) $criteria->add(EntryServerNodePeer::SERVER_NODE_ID, $this->server_node_id);
		if ($this->isColumnModified(EntryServerNodePeer::PARTNER_ID)) $criteria->add(EntryServerNodePeer::PARTNER_ID, $this->partner_id);
		if ($this->isColumnModified(EntryServerNodePeer::CREATED_AT)) $criteria->add(EntryServerNodePeer::CREATED_AT, $this->created_at);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(EntryServerNodePeer::ID, $this->id);
		
		if($this->alreadyInSave)
		{
			if ($this->isColumnModified(EntryServerNodePeer::CUSTOM_DATA))
			{
				if (!is_null($this->custom_data_md5))
					$criteria->add(EntryServerNodePeer::CUSTOM_DATA, "MD5(cast(" . EntryServerNodePeer::CUSTOM_DATA . " as char character set latin1)) = '$this->custom_data_md5'", Criteria::CUSTOM);
					//casting to latin char set to avoid mysql and php md5 difference
				else 
					$criteria->add(EntryServerNodePeer::CUSTOM_DATA, NULL, Criteria::ISNULL);
			}
			
			if (count($this->modifiedColumns) == 2 && $this->isColumnModified(EntryServerNodePeer::UPDATED_AT))
			{
				$theModifiedColumn = null;
				foreach($this->modifiedColumns as $modifiedColumn)
					if($modifiedColumn != EntryServerNodePeer::UPDATED_AT)
						$theModifiedColumn = $modifiedColumn;
						
				$atomicColumns = EntryServerNodePeer::getAtomicColumns();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of EntryServerNode (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setServerNodeId($this->server_node_id);
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     EntryServerNode Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     EntryServerNode Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      EntryServerNode $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(EntryServerNode $copiedFrom)
	{
	 ...
	 *
	 * @return     EntryServerNodePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new EntryServerNodePeer();
		}
		return self::$peer;
	}
